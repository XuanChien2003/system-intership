# USING STREAMS, REDIRECTION, AND PIPES (SỬ DỤNG LUỒNG, CHUYỂN HƯỚNG VÀ ĐƯỜNG ỐNG)
...
### Handling standard output (Xử lý đầu ra chuẩn)
...
### Redirecting Standard Error (Chuyển hướng lỗi chuẩn)
...
### Regulating Standard Input (Điều chỉnh đầu vào chuẩn)
...
## Piping Data between Programs (Truyền dữ liệu qua đường ống giữa các chương trình)
### Pipe là gì?
...
| ---- | --------------------------------- | ------------------------------------------------ |
| 1️  | `grep /sbin/nologin$ /etc/passwd` | Lọc ra các dòng user có shell là `/sbin/nologin` |
| 2️  | `cut -d ":" -f 1`                 | Cắt cột đầu tiên (username) trong mỗi dòng       |
| 3️  | `sort`                            | Sắp xếp danh sách username theo thứ tự alphabet  |
| 4️  | `less`                            | Hiển thị kết quả phân trang dễ xem              |
  
  ![alt text](../Chapter%201/img/5.6.png)
  
Kết quả là một danh sách gọn gàng các username bị khóa shell đăng nhập.

## Using sed (Sử dụng lệnh sed)
...
## Generating Command Lines (Khởi tạo dòng lệnh)
